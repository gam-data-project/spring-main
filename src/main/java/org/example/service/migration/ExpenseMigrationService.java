...
import org.example.repository.ExpenseMigrationMapper;
...
public class ExpenseMigrationService {
    private static final int CHUNK_SIZE = 100;

    private final NewExcelFilterService newPurchaseFilterService;
    private final ExpenseMigrationMapper expenseMigrationMapper;
    ...
    public int saveFilteredExpenses(String fileName,
        ...
        List<ExpenseMigrationDto> expenseList = filteredRows.stream()
     ...
     */
    public ExpenseMigrationDto toNewExpenseInfo(BankTransactionRowDto row,
                                                Long categoryId,
        ...
        return ExpenseMigrationDto.builder()
                .id(0L)
     ...
     */
    @Transactional
    public int saveExpenseListInChunks(List<ExpenseMigrationDto> expenseList) {
        ...
        for (int start = 0; start < expenseList.size(); start += CHUNK_SIZE) {
            ...
            List<ExpenseMigrationDto> chunk = expenseList.subList(start, end);

            log.info("비용 데이터 청크 저장 시작: {} ~ {}", start, end - 1);

            insertedCount += expenseMigrationMapper.insertNewExpenseList(chunk);
        }
