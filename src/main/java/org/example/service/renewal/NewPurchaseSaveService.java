...
import org.example.repository.renewal.NewPurchaseInfoMapper;
...
public class NewPurchaseSaveService {
    private static final int CHUNK_SIZE = 100;

    private final NewExcelFilterService newPurchaseFilterService;
    private final NewPurchaseInfoMapper newPurchaseInfoMapper;
    ...
    public int saveFilteredPurchases(String fileName) throws IOException {
        ...
        List<NewPurchaseInfoDto> purchaseList =
     ...
     */
    @Transactional
    public int savePurchaseListInChunks(List<NewPurchaseInfoDto> purchaseList) {
        ...
        for (int start = 0; start < purchaseList.size(); start += CHUNK_SIZE) {
            ...
            List<NewPurchaseInfoDto> chunk = purchaseList.subList(start, end);

            log.info("매입 데이터 청크 저장 시작: {} ~ {}", start, end - 1);

            insertedCount += newPurchaseInfoMapper.insertNewPurchaseList(chunk);
        }
