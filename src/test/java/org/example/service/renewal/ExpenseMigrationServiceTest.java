...
import org.example.service.migration.ExpenseMigrationService;
...
@Slf4j
@SpringBootTest
public class ExpenseMigrationServiceTest {
    ...
    @Autowired
    private ExpenseMigrationService expenseMigrationService;

    @Test
    @DisplayName("계좌 거래 중 비용 데이터를 DB에 저장한다")
    void saveFilteredExpenses_success() throws IOException {
        int insertedCount = expenseMigrationService.saveFilteredExpenses(
                fileName,
                categoryId,
                expenseType
        );
    ...
    void toNewExpenseInfoList_success() throws IOException {
        ...
        List<ExpenseMigrationDto> expenseList = filteredRows.stream()
                .map(row -> expenseMigrationService.toNewExpenseInfo(row, categoryId, expenseType))
                .toList();
